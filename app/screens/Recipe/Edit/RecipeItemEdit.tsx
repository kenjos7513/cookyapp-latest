...
import styles from "./stylesheets/RecipeItemEditStyleSheet";
...
function RecipeItemEdit({ navigation }: any): React.JSX.Element {
    ...
    const [preparationTime, setTime] = useState(params.preparation_time)
    const [numOfServings, setNumOfServings] = useState(parseInt(params.number_of_servings).toString())
    const [ingredients, setIngredients] = useState<string>(params.ingredients)
    const [procedure, setProcedure] = useState<string>(params.procedure)

    const [cancelModal, showCancelModal] = useState<boolean>(false)


    

    const _closeCancelModal =() => {
        showCancelModal(false)
    }

    const _closeCancelModalWithYes = () => {
        showCancelModal(false)
        _cancelEdit()
    }
    ...
    const _editRecipe = () => {
        ...
        //call recipe edit 
        recipeServices.editRecipe(currentToken, {
            name,
    ...
    return (
        <SafeAreaView onLayout={_onLayout} style={{flexDirection: 'column', backgroundColor: StyleConfig.whiteColor, height, width}}>
            <ScrollView style={{width: '100%'}}>
                ...
                <GenericModal closeText='close' show={errorModal} closeModal={_closeErrorModal} message='An error occured while trying to update recipe.' title='Save Error'/>
                ...
                <CancelModal closeModal={_closeCancelModal} show={cancelModal} name={params.name} closeModalWithYes={_closeCancelModalWithYes} />
